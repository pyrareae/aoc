...
"part one".heading
data = DATA.readlines.map &:chomp # clean data
pairs = data.map{[_1[..._1.length/2],_1[_1.length/2...]]} #split by 2
def value_of(char)= ['a'..'z', 'A'..'Z'].map(&:to_a).flatten.index(char)+1

pairs.map do |(a,b)|
  # find shared    ||  assign values
  (a.chars & b.chars).map{value_of _1}.sum
end.sum.ps

"part two".heading

data.each_slice(3).map do |team|
  value_of team.map(&:chars).reduce{|a,b| a & b}.first
end.sum.ps
